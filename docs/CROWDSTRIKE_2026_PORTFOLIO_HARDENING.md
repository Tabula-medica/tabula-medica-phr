...
| **Per-PR + daily dependency scan** (`npm audit --audit-level=high` gate + OSV-Scanner report) | ✅ `.github/workflows/security-scan.yml`; first run already forced medplum 5.x, fast-uri, nanoid fixes | 🔁 | 🔁 | 🔁 | 🔁 |
...
| `.github/workflows/security-scan.yml` | `npm audit --omit=dev --audit-level=high` (blocking) + OSV-Scanner full-severity table (report-only, ratchet to blocking once clean) on PR, push to `main`, and daily 06:17 UTC; runs the three new security test suites. | On |
