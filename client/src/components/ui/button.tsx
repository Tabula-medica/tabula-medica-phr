...
const buttonVariants = cva(
  // Focus ring matches the unified `*:focus-visible` baseline in index.css
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    if (size === "icon" && !props["aria-label"] && !props["aria-labelledby"] && !props.title) {
      if (process.env.NODE_ENV === "development") {
        console.warn("[a11y] Icon-only <Button> missing aria-label, aria-labelledby, or title.")
      }
    }
    if (size === "icon" && !props["aria-label"] && props.title) {
      a11yProps["aria-label"] = props.title
    }
    return (
      <Comp
