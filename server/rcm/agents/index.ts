...
// stale approvals for the same action) can otherwise both observe "open" before either writes
// back, causing duplicate or conflicting financial actions on the same denial.
...
const fileCorrectedClaim: Tool<{ claimId: string; denialId: string; amount: number }, unknown> = {
  ...
  async run(input, ctx) {
    ...
    try {
      ...
      if (!d || d.status !== "open") throw new Error(`Denial ${input.denialId} is no longer open — this approval is stale`);
      await ctx.store.upsertDenial(ctx.tenantId, { ...d, status: "in-progress" });
